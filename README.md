<!-- HEADER ANIMATION -->
![CyberpunkHeader](https://raw.githubusercontent.com/halfrost/halfrost/master/icons/header_.png)

<div align="center">
  
# 『 ₣ⱠØ₩₴₮Ӿ₮Ɇ の 』
## ⚡ Computer Science & Engineering Student ⚡

</div>

<div align="center" style="background-color: #0D1117; padding: 10px; border-radius: 10px;">
  
[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=00F6FF&width=435&lines=Building+the+digital+future;Exploring+new+technologies;Error+404%3A+Sleep+not+found)](https://git.io/typing-svg)

</div>

<!-- ABOUT ME SECTION -->
<div align="center">
<table style="background-color: #0D1117; border: 1px solid #30363d; border-radius: 6px;">
  <tr>
    <td style="background-color: #161B22; color: #c9d1d9;">
      <h2 style="color: #00f6ff;">// SYSTEM INFO</h2>
      <ul>
        <li>🔮 <b style="color: #00f6ff;">Currently mastering:</b> Python, JavaScript</li>
        <li>🌐 <b style="color: #00f6ff;">System status:</b> Online and learning</li>
        <li>📡 <b style="color: #00f6ff;">Next objective:</b> Full-stack development</li>
      </ul>
    </td>
    <td style="background-color: #161B22;">
      <img src="https://camo.githubusercontent." width="250px" alt="cyberpunk animation">
    </td>
  </tr>
</table>
</div>

<!-- TECH STACK -->
<h2 align="center" style="color: #00f6ff;">＜/＞ TECH STACK</h2>

<div align="center">
  <table style="background-color: #0D1117; border: 1px solid #30363d; border-radius: 6px;">
    <tr>
      <td align="center" style="background-color: #161B22; border: 2px solid #00f6ff; border-radius: 10px; padding: 15px;">
        <div style="display: inline-block; background-color: #3572A5; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="python" width="60" height="60"/>
        </div>
        <div style="display: inline-block; background-color: #F7DF1E; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="javascript" width="60" height="60"/>
        </div>
        <div style="display: inline-block; background-color: #E34F26; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original-wordmark.svg" alt="html5" width="60" height="60"/>
        </div>
        <div style="display: inline-block; background-color: #1572B6; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original-wordmark.svg" alt="css3" width="60" height="60"/>
        </div>
      </td>
    </tr>
  </table>
</div>

<!-- TOOLS -->
<h2 align="center" style="color: #00f6ff;">⚙️ TOOLS</h2>

<div align="center">
  <table style="background-color: #0D1117; border: 1px solid #30363d; border-radius: 6px;">
    <tr>
      <td align="center" style="background-color: #161B22; border: 2px solid #00f6ff; border-radius: 10px; padding: 15px;">
        <div style="display: inline-block; background-color: #007ACC; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vscode/vscode-original.svg" alt="vscode" width="60" height="60"/>
        </div>
        <div style="display: inline-block; background-color: #F1502F; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg" alt="git" width="60" height="60"/>
        </div>
        <div style="display: inline-block; background-color: #333333; padding: 10px; margin: 5px; border-radius: 50%;">
          <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" alt="github" width="60" height="60"/>
        </div>
      </td>
    </tr>
  </table>
</div>

<!-- GITHUB STATS -->
<h2 align="center" style="color: #00f6ff;">📊 SYSTEM DIAGNOSTICS</h2>

<div align="center" style="background-color: #161B22; padding: 10px; border-radius: 10px; margin: 10px 0;">
  <img src="https://github-readme-stats.vercel.app/api?username=flowstxte&show_icons=true&theme=cyberpunk&border_color=00fbff&bg_color=0D1117&title_color=00fbff&icon_color=00fbff&text_color=00f6ff" alt="GitHub Stats">
</div>

<div align="center" style="background-color: #161B22; padding: 10px; border-radius: 10px; margin: 10px 0;">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=flowstxte&theme=black-ice&background=0D1117&stroke=00f6ff&ring=00fbff&fire=00fbff&currStreakNum=00f6ff&sideNums=00f6ff&currStreakLabel=00fbff&sideLabels=00fbff&dates=979797" alt="GitHub Streak Stats">
</div>

<!-- SKILL BARS -->
<h2 align="center" style="color: #00f6ff;">⚡ SKILLS</h2>

<div align="center">
  <table style="background-color: #161B22; border: 1px solid #30363d; border-radius: 6px; color: #c9d1d9;">
    <tr>
      <td><b style="color: #00f6ff;">Python</b></td>
      <td width="420">
        <div style="background-color: #101010; border-radius: 5px; height: 25px; width: 100%;">
          <div style="background-color: #00f6ff; border-radius: 5px; height: 25px; width: 70%;">
            <p align="center" style="color: black; margin: 0; line-height: 25px; font-weight: bold;">70%</p>
          </div>
        </div>
      </td>
    </tr>
    <tr>
      <td><b style="color: #00f6ff;">JavaScript</b></td>
      <td width="420">
        <div style="background-color: #101010; border-radius: 5px; height: 25px; width: 100%;">
          <div style="background-color: #00f6ff; border-radius: 5px; height: 25px; width: 40%;">
            <p align="center" style="color: black; margin: 0; line-height: 25px; font-weight: bold;">40%</p>
          </div>
        </div>
      </td>
    </tr>
    <tr>
      <td><b style="color: #00f6ff;">HTML</b></td>
      <td width="420">
        <div style="background-color: #101010; border-radius: 5px; height: 25px; width: 100%;">
          <div style="background-color: #00f6ff; border-radius: 5px; height: 25px; width: 60%;">
            <p align="center" style="color: black; margin: 0; line-height: 25px; font-weight: bold;">60%</p>
          </div>
        </div>
      </td>
    </tr>
    <tr>
      <td><b style="color: #00f6ff;">CSS</b></td>
      <td width="420">
        <div style="background-color: #101010; border-radius: 5px; height: 25px; width: 100%;">
          <div style="background-color: #00f6ff; border-radius: 5px; height: 25px; width: 58%;">
            <p align="center" style="color: black; margin: 0; line-height: 25px; font-weight: bold;">58%</p>
          </div>
        </div>
      </td>
    </tr>
  </table>
</div>

<!-- Alternative for GitHub skill visualization -->
<div align="center" style="background-color: #161B22; padding: 10px; border-radius: 10px; margin: 10px 0;">
  
![Python](https://img.shields.io/static/v1?label=Python&message=70%&color=00f6ff&style=for-the-badge&logo=python)
![JavaScript](https://img.shields.io/static/v1?label=JavaScript&message=40%&color=00f6ff&style=for-the-badge&logo=javascript)
![HTML](https://img.shields.io/static/v1?label=HTML&message=60%&color=00f6ff&style=for-the-badge&logo=html5)
![CSS](https://img.shields.io/static/v1?label=CSS&message=58%&color=00f6ff&style=for-the-badge&logo=css3)
  
</div>

<!-- CONNECT WITH ME -->
<h2 align="center" style="color: #00f6ff;">📡 UPLINK</h2>

<div align="center" style="background-color: #161B22; padding: 10px; border-radius: 10px; margin: 10px 0;">
  
  [![LinkedIn](https://img.shields.io/badge/-LINKEDIN-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/ullas-sarker-tirtha)
  [![GitHub](https://img.shields.io/badge/-GITHUB-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/flowstxte)
  
</div>

<!-- FOOTER -->
<div align="center">
  <img src="https://raw.githubusercontent.com/trinib/trinib/82213791fa9ff58d3ca768ddd6de2489ec23ffca/images/footer.svg" width="100%">
  
  <p style="background-color: #161B22; padding: 5px; border-radius: 5px; display: inline-block;">
    <img src="https://komarev.com/ghpvc/?username=flowstxte&label=Profile%20views&color=00f6ff&style=flat" alt="Profile Views">
  </p>
</div>
